...
specialc=list(string.punctuation) # lista de Signos de pintuacion estandar

tabla= {
 'A': [28, 30, 25, 12, 46, 41, 63, 55, 14, 26],
 'B': [17],
 'C': [68, 31, 35, 2],
 'D': [72, 57, 54, 44, 81],
...
#Elimina signos de puntuación del mensaje
def preprocesamiento(string):
    for i in specialc:
        string = string.replace(i,"")   # reemplaza signos especiales por ""
    return string
...
def main():
    m= str(input("Ingrese el texto a cifrar:")) #Ingresa el texto
    m=preprocesamiento(m)
    m=m.upper() #Mensaje en mayusculas
